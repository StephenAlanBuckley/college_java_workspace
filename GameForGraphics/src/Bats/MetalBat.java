...
public class MetalBat implements Bats{
	...
	public MetalBat(GLU glu, double x, double y, double z, double direction) {
		coorX = (float)x;
		coorY = (float)y;
		coorZ = (float)z;
	...
	public void move() {
		...
		health += fb.healing();
		if(health < 40){
			setBehavior(new Fleeing());
			isFleeing = true;
		}else if(isFleeing && health > 70){
			setBehavior(new ApproatchPlayer());
			isFleeing = false;
		}
	}
	...
	private void drawBat(GL2 gl, GLU glu){
		gl.glColor3f(0.742f, 0.742f, 0.742f);
		drawBody(gl,glu);
