...
public class YellowBat implements Bats{
	...
	public YellowBat(GLU glu, double x, double y, double z, double direction) {
		coorX = (float)x;
		coorY = (float)y;
		coorZ = (float)z;
		angle = (float)direction;
		fb = new ApproatchPlayer();
	...
	public void draw(GL2 gl, GLU glu) {
		if(isFleeing){
			Explosion.drawAllExplosions(gl, glu);
			health = 0;
		}else{
		gl.glPushMatrix();
	...
	public void move() {
		...
		health += fb.healing();
		if(health < 40){
			Explosion.createNewExplosion(coorX, coorY, coorZ);
			isFleeing = true;
			attack = true;
		}
	}
	...
	public int attacking() {
		// TODO Auto-generated method stub
		if(attack && isFleeing){
			if((playX >= coorX-1.5f && playX <= coorX+1.5f)||(playZ>= coorZ-1.5f && playZ<= coorZ+1.5f)){
				return 10;
			}else{
				return 0;
			}
