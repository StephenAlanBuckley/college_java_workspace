...
public class Famas extends Weapon{
	...
	public Famas(GL2 gl,GLU glu, Game game){
    ...
    famasClip = Building.setupTexture(gl,"famasClip.jpg");
		...
		//this was added to keep the gun's "state"
		clipSize = 24; // max is 24 for the gameframe
		currentClip = clipSize;
		ammo = 60;
		maxAmmo = 90;
		
		try {
			weaponIcon = new ImageIcon(ImageIO.read(FileHelper
					.getFileInputStream("famasIcon.png")));
		} catch (IOException e) {
			System.out.println("Couldn't load Famas Icon");
			e.printStackTrace();
		}
	...
	public void drawGunMove(GL2 gl,GLU glu,double gunM){
		...
		if(isFiring() && ammo>0){ // checks to see you aren't out of ammo
			gl.glPushMatrix();
