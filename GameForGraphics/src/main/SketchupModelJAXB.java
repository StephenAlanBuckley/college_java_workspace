...
public class SketchupModelJAXB {
    ...
    public void generate(GL2 gl, GLU glu) {
        if (model==null) {
            ...
            try {
                ...
                model = (jaxb.COLLADA) unmarshaller.unmarshal(FileHelper.getFileInputStream(filePath + "/" + fileName));
    	...
    	if (images != null) {
    		for (Image image: images) {
    			...
    			if (compiling) {
    	            ...
    	            com.jogamp.opengl.util.texture.Texture texture = Building.setupTexture(gl, filename, filePath);
    ...
    private void emitHeader(String baseName) {
        ...
        emit("import Buildings.Building;");
