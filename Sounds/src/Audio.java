...
// Enhancements needed:
//   - There is noise creeping in from somewhere.  Find it.
...
public class Audio {
	...
	/**
	 * Get an Oscilloscope-like graphical object that draws the waveform.
	 * Updated whenever the left or right channel change.
	 * 
	 * @return JComponent that shows the waveform
	 */
	public JComponent getScope() {
		return scope;
	}
	...
	/**
	 * Saves the current sound into a file. The format will be 44100 samples per
	 ...
	 * 
	 * /** Plays the current sound. / public void play() { // Unfortunately, a
	 * "clip" seems to have a limited size, and // is especially small on a
	 * Macintosh. stop(); try { clip = AudioSystem.getClip(); System.gc();
	 * byte[] bytes = convertToBytes(); clip.open(desiredFormat, bytes, 0,
	 * bytes.length); clip.start(); } catch (LineUnavailableException e) {
	 * System.out.println("Cannot play: " + e); return; } }
	 * 
	 * /** Stops playing the current sound. If no sound is playing, the stop()
	 * method returns without doing anything. / public void stop() { if (clip !=
	 * null) { clip.stop(); clip.flush(); clip.close(); clip = null; } }
	 */
	 ...
	 */
	public void play() { // doesn't use clips, should have no length
							// restrictions.
		stop();
	...
	/*
	 * // This play/stop pair should probably be deleted, it's like the thread
	 * version but // doesn't use threads so it's not interruptible. But, at
	 * least it doesn't use // clips so there is no size restriction.
	 * 
	 * // This method of play works too, but you can't stop it. The write blocks
	 * for a long time. private SourceDataLine line = null; public void play() {
	 * try { AudioFormat format = desiredFormat; line =
	 * AudioSystem.getSourceDataLine(format); line.open(format); byte[] bytes =
	 * convertToBytes(); line.start(); line.write(bytes, 0, bytes.length);
	 * line.drain(); line.close(); line = null; } catch (Exception e) {
	 * System.out.println("Cannot play: " + e); return; } }
	 * 
	 * public void stop() { if (line != null) { line.stop(); line.close(); line
	 * = null; } }
	 */
	...
	public void setLeftChannel(float[] left) {
		...
		scope.setArrays(this.left, this.right);
	...
	/*
	 * // This main is for testing only public static void main(String[] args) {
	 * 
	 * //for (short s = -32768; s<32767; ++s) { // float ii =
	 * signedByteToInt((byte)(s>>>8), (byte)(s&0xff)); // System.out.println(s +
	 * " " + ii); //}
	 * 
	 * //float i1 = signedByteToInt((byte)0x08, (byte)0x07); //float i2 =
	 * signedByteToInt((byte)0xF0, (byte)0x07); //float i2b=
	 * signedByteToInt((byte)0xFF, (byte)0xFF); //float i3 =
	 * signedByteToInt((byte)0x08, (byte)0xF0); //float i4 =
	 * signedByteToInt((byte)0xF0, (byte)0xF0); //float i5 =
	 * unsignedByteToInt((byte)0x08, (byte)0x07); //float i6 =
	 * unsignedByteToInt((byte)0xF0, (byte)0x07); //float i7 =
	 * unsignedByteToInt((byte)0x08, (byte)0xF0); //float i8 =
	 * unsignedByteToInt((byte)0xF0, (byte)0xF0); Audio audio = new
	 * Audio("ManMan-10lbMoustacheIntro.wav"); float[] left =
	 * audio.getLeftChannel(); float[] right = audio.getRightChannel();
	 * audio.checkMaximum(); audio.save("out.wav");
	 * 
	 * audio.play();
	 * 
	 * //audio.play(); try { Thread.sleep(5000); } catch (InterruptedException
	 * e) { e.printStackTrace(); } System.out.println("Stopping clip");
	 * audio.stop(); System.out.println("Clip Stopped"); audio.play(); try {
	 * Thread.sleep(5000); } catch (InterruptedException e) {
	 * e.printStackTrace(); } System.out.println("All done"); //audio.stop();
	 * //audio.stop(); // should not crash }
	 */
}

// Oscilloscope, showing the waveform
class Scope extends JComponent {
	...
	public void setArrays(float[] left, float[] right) {
		this.left = left;
		this.right = right;
		repaint();
	}
	...
	public void paintComponent(Graphics g) {
		...
		erase(g);
		draw(g, left, Color.RED, height*1/4);
		draw(g, right, Color.GREEN, height*3/4);
	...
	private void draw(Graphics g, float[] signal, Color color, int startY) {
		if (signal == null)
			return;
		// System.out.println("Drawing scope");
		g.setColor(color);
		float arraySize = Math.max(left.length, right.length);
		int oldx=0;
		int ymin=startY;
		int ymax=startY;
		for (int i = 0; i < signal.length; i++) {
			int x = (int) (i / arraySize * width);
			int y = (int) (startY - signal[i] * height / 4);
			if (x==oldx) {
				if (y>ymax) ymax=y;
				if (y<ymin) ymin=y;
			} else {
			    g.drawLine(x, ymax, x, ymin); // only 1 draw per x coordinate
			    ymin = ymax = startY;
			    oldx = x;
			}
		}
